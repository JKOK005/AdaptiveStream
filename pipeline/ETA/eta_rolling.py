...
"""
python3 pipeline/ETA/eta_rolling.py \
...
if __name__ == "__main__":
	...
	train_df["full_ts"] 	= train_df.apply(lambda x: x['request_time'] + '-' + "{:02d}".format(x['time_of_day']), axis = 1)
	train_df["epoch_time"] 	= pd.to_datetime(train_df["full_ts"], format = "%Y-%m-%d-%H").map(pd.Timestamp.timestamp)
	train_df 				= train_df.drop(columns = ["request_time", "time_of_day", "full_ts"], axis = 1)
