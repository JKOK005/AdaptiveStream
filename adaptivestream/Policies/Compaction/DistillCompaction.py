...
class DistillCompaction(CompactionPolicy):
    ...
    def compact(self, experts, 
        ...
        student_model = copy(fallback_expert)
        ...
        for teacher_model in K_old_experts:
            logits = teacher_model.infer(self.buffer.get_data())
            teacher_probs = tf.nn.softmax(logits / self.temperature, axis=-1)

            student_loss = student_model.loss(self.buffer.get_data(), self.buffer.get_label())
            student_logits = student_model.infer(self.buffer.get_data())
            ...
            student_model.trained_model.model.train_on_batch(self.buffer.get_data(), self.buffer.get_label(), sample_weight=total_loss)

        # new_fallback_expert = tf.keras.models.clone_model(student_model.trained_model.model)
        
        return student_model, new_experts
